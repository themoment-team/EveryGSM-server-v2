...
`com.github.themoment-team:the-sdk:1.5` is the shared infrastructure library on EveryGSM-server-v2's classpath.
It provides HTTP logging, response wrapping, exception handling, and Swagger auto-configuration out of the box.

> In EveryGSM-server-v2, the project's own `AppExceptionHandler` and direct DTO returns from controllers cover the response/exception layers. The SDK is mostly used for `CommonApiResponse.error(...)` shaping inside the global handler and security filters; `ExpectedException` is the project-local class at `team.themoment.everygsm.server.v2.global.exception.error.ExpectedException`, not the SDK's.
